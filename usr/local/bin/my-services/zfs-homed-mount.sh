...
zfs_homed_mount() {
	...
	if [[ -z "${username}" ]]; then
		...
		{ "$(zfs get mounted "${zfsHomedRoot}/${username}/data" -H -o value)" | grep -q 'yes' && ! cat /proc/mounts | grep -q -F "/dev/mapper/home-${username}" && { [[ "$(homectl inspect "${username}" | grep 'State' | sed -E s/'.*State\: '//)" == 'active' ]] || { dmsetup ls | grep -q -F "home-${username}" && cat /proc/mount | grep -F "/dev/mapper/home-${username}" | grep -q -F "/home/${username}"; }; }; } && return 0
	...
	"$(zfs get mounted "${zfsHomedRoot}/${username}/data" -H -o value)" | grep -q 'yes' && return 0 || return 1
	#{ [[ "$(zfs get mounted "${zfsHomedRoot}/${username}/data" -H -o value)" == 'yes' ]] && cat /proc/mounts | greq -q -F "/home/${username}/.identity"; }  && return 0
	...
	# [[ -n "${username}" ]] && homectl list | grep -q -E '^'"${username}" && [[ "$(homectl inspect "${username}" | grep 'State' | sed -E s/'.*State\: '//)" == 'active' ]] && dmsetup ls | grep -q -F "home-${username}"; } || return 1
...
dbus-monitor --monitor --system "path='/org/freedesktop/home1',member='ActivateHome'" | while read -r dmsg; do 
	...
	# line has username. Pass it to tmpfile_wrapper and unset activateFlag
